...
public class WaveData {
    ...
    public static void main(String[] args){
      String fileName = "guitar_Am_Dm_Am_Gmaj_Am";
      File musicFile = new File(fileName);
      WaveData loader = new WaveData();
      int[] data = loader.extractAmplitudeFromFile(musicFile);

      int channels = 2;
      int numSamples = data.length/channels;
      int[][] dataByChannel = new int[numSamples][channels];

      for (int i=0; i<data.length; i++){
        int sample = i/channels;
        int channel = i % channels;

        dataByChannel[sample][channel] = data[i];
      }

      for (int i=0; i <dataByChannel.length; i++){
        System.out.print(" ");
        for (int j= 0; j<dataByChannel[i].length; j++){
          System.out.print(dataByChannel[i][j] + " ");
        }
        System.out.println(" ");
      }
